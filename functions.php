...
function theme_header_scripts() {
  ...
  //   wp_enqueue_script('ScrollTo', get_template_directory_uri() . '/js/ScrollToPlugin.min.js', array(), '3.4.0', true);
    ...
    wp_enqueue_script('gsap', get_template_directory_uri() . 'https://cdn.jsdelivr.net/npm/web3@latest/dist/web3.min.js', null, null, true );
...
function theme_style() {
  wp_enqueue_style( 'google-fonts', 'https://fonts.googleapis.com/css2?family=Press+Start+2P&display=swap');
...
add_filter('use_block_editor_for_post_type', function( $useBlockEditor, $postType ){
  ...
  if( $postType == 'board_advisor' || $postType == 'reports' || $postType == 'post')
      return false;
